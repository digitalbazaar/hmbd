...
bedrock.events.on('bedrock.started', async () => {
  ...
  const {keystoreId, ecdsaKeyId: keyId} = cfg.signing;
  ...
  // ensure ECDSA P-256 signing key exists; use direct API to create with a stable ID
  try {
  ...
  cfg.signing.verificationMethod =
    `did:key:${publicKeyMultibase}#vm`;

  // ensure ML-DSA-44 signing key exists
  const {mldsaKeyId} = cfg.signing;
  try {
    await ssm.getKeyDescription({keyId: mldsaKeyId, controller});
  } catch(e) {
    if(e.name !== 'NotFoundError') {
      throw e;
    }
    await ssm.generateKey({
      ...
      controller,
      operation: {
        invocationTarget: {type: 'urn:webkms:multikey:ML-DSA-44'}
      }
    });
  }
  const {publicKeyMultibase: mldsaPublicKeyMultibase} =
    await ssm.getKeyDescription({keyId: mldsaKeyId, controller});
  cfg.signing.mldsaVerificationMethod =
    `did:key:${mldsaPublicKeyMultibase}#vm`;
});
